...
export default function Home() {
  ...
  return (
    <div style={{ fontFamily: 'inherit', overflowX: 'hidden' }}>
      ...
      {/* ══ HERO ══ */}
      <section style={{ position:'relative',minHeight:'100vh',background:'#05051a',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden' }}>
        <div className="orb-1" style={{ position:'absolute',width:700,height:700,borderRadius:'50%',background:'radial-gradient(circle,rgba(99,102,241,0.24) 0%,transparent 70%)',top:'-180px',left:'-200px',pointerEvents:'none' }} />
        ...
        <div style={{ position:'absolute',inset:0,backgroundImage:'linear-gradient(rgba(99,102,241,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(99,102,241,0.05) 1px,transparent 1px)',backgroundSize:'64px 64px',pointerEvents:'none' }} />

        <div style={{ position:'relative',zIndex:2,textAlign:'center',padding:'80px 24px 0',maxWidth:980 }}>
          ...
          <h1 className="hero-title fade-in-up-d1" style={{ fontSize:'clamp(50px,9vw,100px)',fontWeight:900,lineHeight:1.02,letterSpacing:'-4px',margin:'0 0 26px' }}>
        ...
        {/* Floating cards */}
        <div className="float-anim" style={{ position:'absolute',right:'6%',bottom:'18%',background:'rgba(255,255,255,0.07)',backdropFilter:'blur(16px)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:16,padding:'14px 18px',display:'flex',alignItems:'center',gap:12,zIndex:2,minWidth:230 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11,color:'rgba(255,255,255,0.5)' }}>
              <span style={{ color:'#f87171',textDecoration:'line-through',marginRight:5 }}>$109</span>
              <span style={{ color:'#86efac',fontWeight:800 }}>$79.00</span>
            </div>
          </div>
        </div>
        <div className="float-anim" style={{ position:'absolute',left:'5%',bottom:'28%',background:'rgba(255,255,255,0.07)',backdropFilter:'blur(16px)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:16,padding:'12px 16px',display:'flex',alignItems:'center',gap:10,zIndex:2,animationDelay:'1s',minWidth:200 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11,color:'rgba(255,255,255,0.5)' }}>
              <span style={{ color:'#f87171',textDecoration:'line-through',marginRight:5 }}>$189</span>
              <span style={{ color:'#86efac',fontWeight:800 }}>$149.00</span>
            </div>
          </div>
        </div>

        {/* Recently sold badge */}
        <div className="float-anim" style={{ position:'absolute',left:'50%',top:'22%',transform:'translateX(-50%)',background:'rgba(34,197,94,0.12)',backdropFilter:'blur(12px)',border:'1px solid rgba(34,197,94,0.25)',borderRadius:100,padding:'7px 16px',display:'flex',alignItems:'center',gap:8,zIndex:2,animationDelay:'0.7s',whiteSpace:'nowrap' }}>
          <span style={{ width:7,height:7,borderRadius:'50%',background:'#22c55e',display:'inline-block',animation:'none' }} />
          <span style={{ fontSize:12,color:'rgba(255,255,255,0.8)',fontWeight:600 }}>12 people bought this today</span>
        </div>
      ...
      {/* ══ TRUST / MARQUEE BAR ══ */}
      <div style={{ background:'#fff',borderBottom:'1px solid #e8eaf0',overflow:'hidden',padding:'0' }}>
          ...
          {[...Array(2)].map((_,ri)=>(
            <div key={ri} style={{ display:'flex',alignItems:'center',gap:0 }}>
              {[
                ...
                <div key={text} style={{ display:'flex',alignItems:'center',gap:8,padding:'14px 32px',borderRight:'1px solid #f0f0f5' }}>
                  <span style={{ fontSize:16 }}>{icon}</span>
                  <span style={{ fontSize:13,fontWeight:600,color:'#374151',whiteSpace:'nowrap' }}>{text}</span>
                </div>
              ))}
            </div>
          ))}
      ...
      {/* ══ FLASH DEALS ══ */}
      <section style={{ background:'linear-gradient(90deg,#1e1b4b,#2d1b6e,#1e1b4b)',padding:'18px 24px' }}>
        <div style={{ maxWidth:1100,margin:'0 auto',display:'flex',alignItems:'center',gap:18,flexWrap:'wrap' }}>
          ...
          <div style={{ marginLeft:'auto',display:'flex',alignItems:'center',gap:10 }}>
            <span style={{ fontSize:12,color:'rgba(255,255,255,0.5)' }}>Up to 30% off</span>
            <button onClick={()=>navigate('/products')} style={{ background:'#f59e0b',color:'#000',border:'none',borderRadius:8,padding:'8px 20px',fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:'inherit' }}>
              See all deals →
            </button>
          </div>
        </div>
      ...
      <section style={{ maxWidth:1100,margin:'0 auto',padding:'80px 24px 50px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:16 }}>
          {[
            ...
            <div key={cat.name} className="hover-card img-zoom"
              onClick={()=>navigate('/products?cat=' + encodeURIComponent(cat.name))}
              style={{ position:'relative',height:200,borderRadius:18,overflow:'hidden',cursor:'pointer' }}>
              <img src={cat.img} alt={cat.name} />
              <div className="cat-overlay" />
              <div style={{ position:'absolute',inset:0,display:'flex',flexDirection:'column',justifyContent:'flex-end',padding:'18px 20px',zIndex:1 }}>
                <span style={{ fontSize:24,marginBottom:4 }}>{cat.icon}</span>
                <span style={{ fontSize:10,color:'rgba(255,255,255,0.6)',fontWeight:700,textTransform:'uppercase',letterSpacing:'1px' }}>
                  {PRODUCTS.filter(p=>p.category===cat.name).length} products
                </span>
                <span style={{ fontSize:20,fontWeight:800,color:'#fff',letterSpacing:'-0.5px',lineHeight:1.1 }}>{cat.name}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ maxWidth:1100,margin:'0 auto',padding:'10px 24px 80px' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(245px,1fr))',gap:24 }}>
          {featured.map(p=>(
      ...
      <section style={{ maxWidth:1100,margin:'0 auto',padding:'0 24px 80px' }}>
        <div style={{ background:'linear-gradient(135deg,#6366f1 0%,#8b5cf6 50%,#a855f7 100%)',borderRadius:24,padding:'48px 52px',position:'relative',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24 }}>
          <div style={{ position:'absolute',top:-60,right:-60,width:240,height:240,borderRadius:'50%',background:'rgba(255,255,255,0.07)' }} />
          <div style={{ position:'absolute',bottom:-40,right:200,width:160,height:160,borderRadius:'50%',background:'rgba(255,255,255,0.05)' }} />
          <div style={{ position:'relative',zIndex:1 }}>
            <div style={{ fontSize:12,fontWeight:800,color:'rgba(255,255,255,0.7)',textTransform:'uppercase',letterSpacing:'2px',marginBottom:10 }}>Limited time</div>
            <h2 style={{ fontSize:'clamp(24px,4vw,40px)',fontWeight:900,color:'#fff',letterSpacing:'-1px',margin:'0 0 10px',lineHeight:1.1 }}>
              Up to 30% off<br />selected items
            </h2>
            <p style={{ fontSize:15,color:'rgba(255,255,255,0.65)',margin:0 }}>Use code <strong style={{ color:'#fde68a',fontFamily:'monospace' }}>SAVE30</strong> at checkout</p>
          </div>
          <button onClick={()=>navigate('/products')} style={{ position:'relative',zIndex:1,background:'#fff',color:'#6366f1',border:'none',borderRadius:14,padding:'16px 36px',fontSize:16,fontWeight:900,cursor:'pointer',fontFamily:'inherit',letterSpacing:'-0.3px',boxShadow:'0 4px 20px rgba(0,0,0,0.15)',flexShrink:0,transition:'transform 0.2s,box-shadow 0.2s' }}
      ...
      {/* ══ FOOTER ══ */}
      <footer style={{ background:'#030312',color:'rgba(255,255,255,0.35)',padding:'52px 24px 32px',borderTop:'1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:36,marginBottom:48 }}>
            <div>
              ...
              <p style={{ fontSize:13,lineHeight:1.7,color:'rgba(255,255,255,0.3)',margin:0 }}>Premium tech gear for creators who demand the best.</p>
            </div>
            {[
              ...
              <div key={col.title}>
                <div style={{ fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'1.2px',color:'rgba(255,255,255,0.5)',marginBottom:14 }}>{col.title}</div>
                <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                  {col.links.map(([path, [label]])=>(
                    <button key={label} onClick={()=>navigate(path)}
                      style={{ background:'none',border:'none',color:'rgba(255,255,255,0.32)',fontSize:13,cursor:'pointer',fontFamily:'inherit',padding:0,textAlign:'left',transition:'color 0.2s' }}
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.05)',paddingTop:24,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12 }}>
            <span style={{ fontSize:11 }}>© 2025 ShopFlow Demo Store · <em>Monitored by FlowTest Analytics</em></span>
...
function StatsBar() {
  ...
  return (
    <section style={{ background:'#f5f6ff',borderTop:'1px solid #e4e6f5',borderBottom:'1px solid #e4e6f5',padding:'52px 24px' }}>
...
function ProductCard({ p, added, onAdd }) {
  ...
  return (
    <div data-testid="product-card" className="hover-card"
      ...
      style={{ background:'#fff',border:'1px solid #e8eaf0',borderRadius:20,overflow:'hidden',cursor:'pointer',boxShadow:'0 2px 12px rgba(0,0,0,0.04)' }}>

      <div className="img-zoom" style={{ position:'relative',aspectRatio:'4/3' }} onClick={()=>navigate(`/products/${p.slug}`)}>
        <img src={p.image} alt={p.name} loading="lazy" />
        {p.badge && (
          <span style={{ position:'absolute',top:10,left:10,background:p.badge==='Hot'?'linear-gradient(135deg,#ef4444,#f97316)':p.badge==='New'?'linear-gradient(135deg,#22c55e,#16a34a)':p.badge==='Popular'?'linear-gradient(135deg,#f59e0b,#d97706)':'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',fontSize:10,fontWeight:900,padding:'3px 10px',borderRadius:100,letterSpacing:'0.3px' }}>
            {p.badge}
          </span>
        )}
        {discount>0 && (
          <span style={{ position:'absolute',top:p.badge?40:10,left:10,background:'rgba(239,68,68,0.9)',color:'#fff',fontSize:10,fontWeight:900,padding:'3px 9px',borderRadius:100 }}>-{discount}%</span>
        )}
        {hovered && (
          <div style={{ position:'absolute',inset:0,background:'rgba(5,5,26,0.06)',display:'flex',alignItems:'center',justifyContent:'center' }}>
            <span style={{ background:'rgba(255,255,255,0.96)',color:'#6366f1',fontSize:12,fontWeight:800,padding:'8px 18px',borderRadius:22,boxShadow:'0 4px 16px rgba(0,0,0,0.12)' }}>Quick view →</span>
          </div>
        )}
      </div>

      <div style={{ padding:'16px 18px' }}>
        <div data-testid="product-category" style={{ fontSize:10,color:'#6366f1',fontWeight:800,textTransform:'uppercase',letterSpacing:'1px',marginBottom:5 }}>{p.category}</div>
        <div style={{ fontSize:15,fontWeight:800,marginBottom:7,lineHeight:1.3,color:'#0c0c1d',letterSpacing:'-0.3px' }} onClick={()=>navigate(`/products/${p.slug}`)}>
          {p.name}
        </div>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:14 }}>
          ...
          {p.originalPrice && <span style={{ fontSize:13,color:'#9ca3af',textDecoration:'line-through' }}>{fmt(p.originalPrice)}</span>}
        </div>
        <button onClick={onAdd} data-testid="add-to-cart"
          style={{ width:'100%',background:added?'#22c55e':'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',border:'none',borderRadius:11,padding:'11px',fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:'inherit',transition:'all 0.22s',boxShadow:added?'0 4px 14px rgba(34,197,94,0.4)':'0 4px 14px rgba(99,102,241,0.3)' }}>
    ...
    <section style={{ background:'#0c0c20',padding:'96px 24px' }}>
      <div style={{ maxWidth:1100,margin:'0 auto' }}>
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:22 }}>
          {pillars.map(p=>(
            <div key={p.title}
              style={{ background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:18,padding:'34px 30px',transition:'all 0.3s ease' }}
    ...
    <section style={{ background:'#f8f9ff',padding:'88px 24px',borderTop:'1px solid #e8eaf5' }}>
      <div style={{ maxWidth:1100,margin:'0 auto' }}>
        <div style={{ textAlign:'center',marginBottom:52 }}>
          <h2 style={{ fontSize:32,fontWeight:900,color:'#0c0c1d',letterSpacing:'-0.8px',margin:'0 0 10px' }}>Loved by 50,000+ customers</h2>
